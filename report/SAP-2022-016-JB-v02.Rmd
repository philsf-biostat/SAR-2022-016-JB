...
Modelos de regressão linear múltipla serão usados para estimar a associação entre a taxa mensal de cobertura vacinal e a taxa de internação por SRAG.
Para avaliar o impacto de cada covariável usada no controle, planeja-se criar diversos modelos em complexidade crescente.
A estimativa bruta da associação entre a cobertura vacinal e a taxa de internações será avaliada com uma regressão linear simples.
